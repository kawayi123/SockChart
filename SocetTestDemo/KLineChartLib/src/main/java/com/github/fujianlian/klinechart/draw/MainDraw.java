...
public class MainDraw implements IChartDraw<ICandle> {
    ...
    private float mBarWidth = 6;
    private float mBarRatio = 0;
    private float mCandleLineWidth = 0;
    private Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
    private Paint mLinePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    private Paint mRedPaint = new Paint(Paint.ANTI_ALIAS_FLAG);

    private Paint barKChartPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    private Paint barKlinePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    ...
    public MainDraw(BaseKLineChartView view) {
        ...
        mContext = context;
        mRedPaint.setColor(Color.rgb(210,41,29));
        mGreenPaint.setColor(Color.rgb(70,168,61));
...
//                    //画ma30
                ...
                break;
            case BAR_TYPE:
                drawBar(view,canvas,lastX, curPoint.getHighPrice(), curPoint.getLowPrice(), curPoint.getOpenPrice(), curPoint.getClosePrice());
                if (status == Status.MA) {
//                    //画ma5
                    if (lastPoint.getMA5Price() != 0) {
                        view.drawMainLine(canvas, ma5Paint, lastX, lastPoint.getMA5Price(), curX, curPoint.getMA5Price());
...
//            }
    ...
    private void drawBar(BaseKLineChartView view, Canvas canvas,float x, float highPrice, float lowPrice, float openPrice, float closePrice) {
        ...
        barKlinePaint.setStrokeWidth(3f);
        barKChartPaint.setStrokeWidth(6f);

        float spaceWidth = 6 * 0.5f;
        float r = spaceWidth / 2f;
        float lineR = (float) 2f;
        float xValue = 7f;
        if (openPrice > closePrice) {//down
            barKChartPaint.setColor(Color.rgb(70,168,61));
            barKlinePaint.setColor(Color.rgb(70,168,61));
            canvas.drawRect(x + r, close, x + r, open, barKChartPaint);
            canvas.drawRect(x - lineR, high, x + lineR, low, barKChartPaint);
            canvas.drawLine(x-xValue , high + 15,  x , high + 15, barKlinePaint);
            canvas.drawLine(x , low - 15,  x+ xValue , low - 15, barKlinePaint);

        } else if (openPrice < closePrice) {//up
            barKChartPaint.setColor(Color.rgb(210,41,29));
            barKlinePaint.setColor(Color.rgb(210,41,29));
            canvas.drawRect(x + r, open, x + r, close, barKChartPaint);
            canvas.drawRect(x - lineR, high, x + lineR, low, barKChartPaint);
            canvas.drawLine(x-xValue , open,  x , open, barKlinePaint);
            canvas.drawLine(x , open - 15,  x+ xValue , open - 15, barKlinePaint);


        } else {
            barKChartPaint.setColor(Color.rgb(210,41,29));
            barKlinePaint.setColor(Color.rgb(210,41,29));
            canvas.drawRect(x + r, open, x + r, close + 1, barKChartPaint);
            canvas.drawRect(x - lineR, high, x + lineR, low, barKChartPaint);
            canvas.drawLine(x-xValue , close + 10,  x , close +10, barKlinePaint);
            canvas.drawLine(x , close - 10,  x+ xValue , close - 10, barKlinePaint);
        }
